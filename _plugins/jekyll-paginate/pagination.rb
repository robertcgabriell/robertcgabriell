  ...
  module Generators
    class Pagination < Generator
      ...
      def generate(site)
        if Pager.pagination_enabled?(site)
		  for category in site.categories.keys
          	if template = self.class.template_page(site, category)
      ...
      def paginate(site, page, category)
        ...
        pages = Pager.calculate_pages(all_posts, site.config['paginate_per_category'].to_i)
      ...
      def self.first_page_url(site, category)
        if page = Pagination.template_page(site, category)
          page.url
        else
          nil
        end
      ...
      def self.template_page(site, category)
        site.pages.select do |page|
          Pager.pagination_candidate?(site.config, page, category)
        end.sort do |one, two|
