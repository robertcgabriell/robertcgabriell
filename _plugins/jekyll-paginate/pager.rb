  ...
  module Generators
    class Pager
      ...
      def self.pagination_enabled?(site)
       !site.config['paginate_per_category'].nil? &&
         site.pages.size > 0
      end
      ...
      #
      # Returns true if the
      def self.pagination_candidate?(config, page, category)
        page_dir = File.dirname(File.expand_path(remove_leading_slash(page.path), config['source']))
        paginate_format = remove_leading_slash(config['paginate_path'])
		if category != config['paginate_root_category']
          paginate_path = File.expand_path(paginate_format, config['source'] + '/' + category)
		else
          paginate_path = File.expand_path(paginate_format, config['source'])
		end
      ...
      def self.paginate_path(site, num_page, category)
        ...
        ensure_leading_slash(category + '/' + format)
      end
      ...
      def initialize(site, page, all_posts, num_pages = nil, category)
        @page = page
        @per_page = site.config['paginate_per_category'].to_i
